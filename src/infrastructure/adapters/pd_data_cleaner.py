...
class PdDataCleaner(DataCleanerProvider):
    ...
    def clean_raw_data(self,
        ...
        df = pd.DataFrame(df.groupby("nca_number", as_index=False).agg({
        ...
        records = self._convert_df_to_object_list(df_records,
                                                  Record)
        allocations = self._convert_df_to_object_list(df_allocations,
                                                      Allocation)
    ...
    def _convert_raw_to_df(self, raw_rows: List[List[str | None]]):
        table_header = [item.lower().replace(
            " ", "_") if item else "" for item in raw_rows[0]]
    ...
    def _create_df_allocations(self, df: pd.DataFrame):
        ...
        df_allocations = pd.DataFrame(df_allocations.dropna(
            subset=self.allocation_columns[1:], how="all")
        )
        df_allocations = df_allocations.fillna(
    ...
    def _convert_df_to_object_list(self,
                                   df: pd.DataFrame,
                                   object_class:
                                   type[Record] |
                                   type[Allocation]
